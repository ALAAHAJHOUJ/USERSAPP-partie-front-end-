...
function Modalmodifier({succes1,erreur1,annule,idUser1}) {
    ...
    const valider=()=>{
          ...
          let  valider=true
              if(ref1.current.value.trim()!='')
              {
                ...
                {setError((prev)=>{return {...prev,champs1:"Syntaxe invalide"}});valider=false}
                else 
                setError((prev)=>{return {...prev,champs1:undefined}})
              }
              else 
              {
                setError((prev)=>{return {...prev,champs1:"champs requis"}})
                valider=false
              }
              ...
              if(!regex1.test(ref2.current.value))
                {setError((prev)=>{return {...prev,champs2:"Syntaxe invalide"}});valider=false;}
              else 
                setError((prev)=>{return {...prev,champs2:undefined}})
              }
              else 
              {
                setError((prev)=>{return {...prev,champs2:"champs requis"}})
                valider=false
              }
              ...
              if(!regex2.test(ref3.current.value))
                {setError((prev)=>{return{...prev,champs3:"syntaxe invalide"}});valider=false;}
              else 
                setError((prev)=>{return {...prev,champs3:undefined}})
              }
              else 
              {
              setError((prev)=>{return {...prev,champs3:"champs requis"}})
              valider=false
              }
                  ...
                  if(!regexTel.test(champsTel.current.value))
                      {setError((prev)=>{return {...prev,champsTel:"numéro invalide"}});valider=false;}
                  else 
                  ...
                  {
                  setError((prev)=>{return {...prev,champsTel:"champs requis"}})
                  valider=false
                  }
    ...
    const ModifierUser=async()=>{
      const resultat=await fetch("http://localhost:8000/ModifierUser/",{credentials:"include",method:"POST",body:JSON.stringify({idUser:idUser1,nom:ref1.current.value,prenom:ref2.current.value,email:ref3.current.value,telephone:champsTel.current.value}),headers:{'Content-Type': 'application/json'}})

      const resultat1=await resultat.text()

      if(resultat1!='opération passée avec succes')
      {
    ...
    <div className='fixed w-full h-full top-0 left-0 flex justify-center items-start sup bg-[#1f1f1f82] overflow-y-auto '>
      ...
      <div  className='rounded-[15px] shadow-[0_0_27.9px_10px_rgba(255,255,255,0.25)] bg-white w-[90%] min-[600px]:w-[400px] pt-[20px] pb-[20px] flex content-start  justify-center gap-[20px] flex-wrap'>
        <span className='font-[700] italic text-[20px]'>Modifier Utilisateur</span>
           <div className='w-full flex flex-wrap justify-center gap-[10px] items-center'>
            <div className='w-[80%]  h-fit min-h-[100px]  flex justify-center flex-wrap '>
              ...
              {error.champs1?<span className='text-red-700 h-[40px] '>{error.champs1}</span>:<></>}
            </div>
            ...
            <div className='w-[80%] h-fit min-h-[100px] flex justify-center flex-wrap'>
              ...
              {error.champs2?<span className='text-red-700 h-[40px]'>{error.champs2}</span>:<></>}
            </div>
            ...
            <div className='w-[80%] h-fit min-h-[100px]  flex justify-center flex-wrap'>
              ...
              {error.champs3?<span className='text-red-700 h-[40px]'>{error.champs3}</span>:<></>}
            </div>
            ...
            <div className='w-[80%] h-fit min-h-[100px]  flex justify-center flex-wrap'>
              ...
              {error.champsTel?<span className='text-red-700 h-[40px]'>{error.champsTel}</span>:<></>}
            </div>
